package serializingAndDeserializing.applyingWriteReplace;
import java.io.*;
import java.util.*;
...
/**
      ...
      Maintenant, que faire si nous voulons écrire un enregistrement d'employé
      sur le disque[to write an Employee record to disk] mais
      que nous ne faisons pas totalement confiance à l'instance que nous détenons ?
      Par exemple, nous voulons toujours écrire la version de l'objet dans le pool
      plutôt que cette instance. Par construction, il ne devrait y avoir qu'une seule version
      de cet objet en mémoire, mais pour cet exemple,
      supposons que nous ne sommes pas sûrs à 100 % de cela.
      La méthode writeReplace() est exécutée avant(before)
      writeObject() et nous permet de remplacer l'objet sérialisé.
...
class Employee implements Serializable{
    ...
    /**
     * Cette implémentation vérifie si l'objet est trouvé dans le pool.
     * Si elle est trouvée dans le pool, cette version est envoyée pour sérialisation ;
     * sinon, l'instance actuelle est utilisée.
     * Nous pourrions également mettre à jour cet exemple pour l'ajouter au pool s'il manque d'une manière ou d'une autre.
     */
