 ...
 * Given the following code, which method declarations can be inserted at line 1 without any problems?
   ...
   It is illegal for a class to have two methods having same name and having same type of input parameters in the same order.
    ...
    1. Option 1 is wrong because, then both the methods will be same (as their method name and the class/type and order of the input parameters will be same).
      So this amounts to duplicate method which is not allowed.
    ...
    5 is not valid because it leads to duplicate method(as their method name and the class/type and order of the input parameters will be same).
    Note that as mentioned in the comments, return type does not matter.
 */
