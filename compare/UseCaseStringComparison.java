 ...
 * You may assume there are no null values. Which method could implement such a comparator?
   ...
   Unlike the equals() method, the method in Comparator˂String˃ takes the type being compared as the parameters when using generics.
   ...
   The negative value says that s1 should sort first, which is correct, because we want the longest String first.

 */
public class UseCaseStringComparison implements Comparator<String> {
    public int compare(String s1, String s2) {
        return s1.length() - s2.length();
    }
}