...
/**
 Which statements about the following class are correct? (Choose all that apply.)
  ...
  The JVM controls when and if garbage collection happens,
  with no guarantees to the application code when it might occur,
  making options G and H both incorrect.

  To solve this problem, we need to track the number of references
  for each object created on lines 4 through 6.

  The object created on line 4 has two references to it after line 7
  and drops down to just one reference, hopper, after line 8.

  After line 9, the object created on line 4 now has two references, hopper and silly,
  although the object created on line 5 is now inaccessible, making option C correct.

  After line 10, the object created on line 4 has one reference,
  silly, while the object created on line 6 also has two references,
  hopper and snuggly.
  One of those references for this object is dropped after line 11.

  The result is the method ends with one reference
  to the object created on line 4, silly,and one reference
  to the object created on line 6, hopper.

  These objects go out of scope when the method ends, making options B and F correct.

 */
public class Bunny implements Hops { // 2:
