...
/**
  ...
  B. A call to n.makeThisNull() marks the private instance of M for garbage collection.
      ...
      However, remember that, in Java, every call is pass by value. Therefore, when makeItNull(m) is called,
      the value of m is passed to the method and not m itself. The method parameter pM is therefore is not same
      as m but a new variable that points to the same object as the one pointed to by m.
      It is that variable pM which is set to null. The original m still keeps pointing to the same object as it was.
   ...
   D.
      private members of a class become eligible for garbage collection only when the instance
      of the class itself becomes eligible for garbage collection.
      ...
      For example, in the following code, the Object instance referred to by 'o',
       can be made eligible for garbage collection by calling setNull(), even if the instance of X itself is not eligible for garbage collection.
