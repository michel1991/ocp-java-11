...
/**
 What will the following code print?
  ...
  -----------------------------------------------------------------------------------------------------------------------------------
    ...
    Although they can compare arrays of primitives as well as reference types, they are most useful for comparing primitive arrays.
    Efficient comparison of primitive arrays is a common requirement in data processing.
    ...
    These methods are heavily overloaded but practically, there are only two flavors that you need to understand
    for the purpose of the exam. The rest are mere replicas; one for each data type. 
    
    Before getting into the details of the methods, you need to understand the meaning of the two words that are used often
    in the description for these methods.
    ...
    you would compare the words letter by letter starting from the first position.
    For example, the word "children" will appear before (and can be considered smaller) than the word "chill"
    because the first four letters of the two words are the same but the letter at the fifth position in "children" (i.e. d)
    comes before (or is smaller than) the letter at the fifth position in "chill" (i.e. l).
    ...
    2. Prefix - This refers to the common part of the two arrays.
    In the above example, "chil" can be called as the prefix while comparing "children" and "chill".
    Similarly, 123 is the prefix while comparing 12345 and 1235.
    ...
    as if by Integer.compare(int, int), at an index within the respective arrays that is the prefix length.
     ...
     (See mismatch(int[], int[]) for the definition of a common and proper prefix.)
    ...
    If the two arrays share a common prefix then the returned index is the length of the common prefix and it follows
    that there is a mismatch between the two elements at that index within the respective arrays.
    If one array is a proper prefix of the other then the returned index is the length of the smaller
    array and it follows that the index is only valid for the larger array. Otherwise, there is no mismatch.
