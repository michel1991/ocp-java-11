...
```txt
    ...
    There is no need to add a dependency on msql jar in myapp.
    ...
    java --module-path datalayer.jar;myapp.jar
    -classpath mysql-connector-java-8.0.11.jar --module abc.myapp
```
...
Basically, hyphens are converted into dots and the version number part is ignored. So, for example,
if you put mysql-connector-java-8.0.11.jar on module path, its module name would be
