...
/**
 ...
 Which of the following are possible outputs when the above program is run?
   ...
   C.  It may print any combination except: a, or b, or a, b, or b, a
      ----
        This is correct because the order of iteration is not known and so the thread that removes "a" and "b",
        may remove them in any order. Thus, the iterator thread may or may not see "a" and/or "b" through its Iterator.
        However, "c" is never removed from the map and so c, will always be printed.
        
   C. It may print any combination except: b, c,
   D. It may print any combination except: a, b,
   ...
   which means any operations done on the ConcurrentHashMap instance may be reflected in the Iterator.
    ...
    Thus, in this case, the thread that is iterating through the entries may or may not see the entries removed from the map by another thread.
    The following is what JavaDoc API description says about ConcurrentHashMap:
    ...
    Retrieval operations (including get) generally do not block, so may overlap with update operations (including put and remove).
     ...
     Similarly, Iterators and Enumerations return elements reflecting the state of the hash table at some point
    ...
    and guarantees to traverse elements as they existed upon construction of the iterator, and may
