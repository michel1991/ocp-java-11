 ...
 * Fill in the blank with the value that allows the annotation to be accessible by the JVM in which it is executed?
    ...
    The @Retention annotation can be applied only to an annotation declaration,
    not a class declaration, making option F the correct answer.
    Applying it to a class will result in a compiler error.
    If Corn were an annotation, though, then RetentionPolicy.RUNTIME would be correct.
 */
