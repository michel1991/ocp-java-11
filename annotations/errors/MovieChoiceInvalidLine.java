...
/**
  Which of the following lines of code do not compile? (Choose all that apply.)
   ...
   There is a Javadoc @deprecated annotation, but that is not the same as the built-in
   Java annotation and is used only within a Javadoc comment.
   ...
   Note that line 5 does compile, as it uses an annotation
   as the type and declares a default value—in this case, an empty array.
 */
@interface Target{
   String[] value();
}
