...
public class SumWithToIntFunction {
    ...
    /**
        ...
        It requires this functional interface, which takes a generic argument and returns int.
         For this reason, option C is correct. Option F is incorrect because
         sum() on an IntStream returns an int, not an OptionalInt.
         Note that the peek() operations in this stream have no effect.


     */
  static void sum(){
        ToIntFunction<Integer> transformer = x -> x; //  _____________ transformer = x -> x;
