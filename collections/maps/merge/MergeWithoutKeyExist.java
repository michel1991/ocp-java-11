 ...
 *  What will the following code print?
    ...
    B. {a=1, b=1, c=3}
    C.  {a=1, b=2}
    D. A NullPointerException will be thrown at run time.
...
of the given remapping function, or removes if the result is null. This method may be of
use when combining multiple mapped values for a key. For example,
...
Parameters:
    key - key with which the resulting value is to be associated value - the non-null value
to be merged with the existing value associated with the key or, if no existing value or
a null value is associated with the key, to be associated with the key remappingFunction
...
Correct(A)
 */
public class MergeWithoutKeyExist {
