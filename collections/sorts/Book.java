...
/**
 Given:
   ...
   A. 
    ...
    The thenComparing method adds on a Comparator to the existing Comparator. This additional Comparator is used only when two objects are determined to be equal by the first Comparator.
    Thus, in the given question, the books will be first sorted by their genre and title (i.e. if the genre of two books is same, then they will be sorted as per their title). This means the list will be ordered as :
    [fiction:The Outsider, non-fiction:Becoming, non-fiction:Uri]
    
    Next, the reversed method generates a Comparator that reverses the order of the Comparator on which it is invoked. Thus, whatever is order of the list produced by Book::getGenre).thenComparing(Book::getTitle), that order is reversed. Therefore, the final order will be :
    [non-fiction:Uri, non-fiction:Becoming, fiction:The Outsider]
