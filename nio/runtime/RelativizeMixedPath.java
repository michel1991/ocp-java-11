 ...
 *  What will the following code fragment print?
   ...
   E.  java.lang.IllegalArgumentException will be thrown
     Note that if one path has a root (for example, if a path starts with a // or c:)
     and the other does not, relativize cannot work and it will throw an IllegalArgumentException
     
You need to understand how relativize works for the purpose of the exam. The basic idea of relativize is
to determine a path, which, when applied to the original path will give you the path that was passed.
...
yields a path that locates the same file as the given path. For example, on UNIX,
...
be "../x". If "b" is a symbolic link then is implementation dependent if "a/b/../x" would locate the same file as "/a/x".

Correct(E)
 */
public class RelativizeMixedPath {
